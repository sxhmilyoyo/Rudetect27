...
def create_parser():
    ...
    parser.add_argument('--label_tags', type=str, default=['NN,NN', 'JJ,NN'],
                        nargs='+',
...
To disable it, pass 'None'""")
    parser.add_argument('--label_min_df', type=int, default=5,
                        help='Minimum document frequency' +
                             'requirement for candidate labels')
    ...
    parser.add_argument('--n_top_words', type=int, default=15,
                        help='Number of topical word to display for each topic'
                        )
...
def get_topic_labels(corpus_path, n_topics,
    ...
    if tag_constraints:
        assert 'tag' in preprocessing_steps, \
            'If tag constraint is applied, pos tagging should be performed'
    ...
    return ranker.top_k_labels(topic_models=model.topic_word_,
                               ...
                               label_models=None,
                               k=n_labels), topic_id2words, model.topic_word_
